...
from feed.templatetags.mentions import mentions

from users.models import User, UserStat, Story, Notification, PostNotification
from .forms import UpdateForm, RepostForm, RepostCommentBox, CommentBox, CreateForm
from .models import *
...
def post_list_view(request):
    # Retrieve a list of users not followed or blocked by the current user, and not the current user
    ...
    # Get the lists of users the current user is following, has blocked, and has muted
    ...
    # Initialize lists for posts and reposts of blocked users
    single_user_posts = []
    single_user_reposts = []

    # Iterate over blocked users to collect their posts and reposts
    ...
    # Retrieve reposts by followed users, excluding posts and reposts from blocked or muted users
    ...
    # Retrieve posts by followed users, excluding posts from blocked or muted users, and liked posts
    ...
    # Add words with length 5 or more to the central list
    for word in string.split():
        if len(word) >= 5:
            central_list.append(word)

    # Count the frequency of words and get the top 7 most common words
    counter = Counter(central_list)
    top_words = counter.most_common(7)

    # Flatten the list of tuples and convert it to a dictionary
    ...
    # Build a list of user ids from the sessions
    for session in sessions:
        data = session.get_decoded()
        uid_list.append(data.get('_auth_user_id', None))

    # Query all logged-in users based on the id list
...
def search(request):
    ...
    if request.method == 'GET':
        ...
        if query is not None:
            ...
            results_people = User.objects.filter(lookups_people).distinct()
